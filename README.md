...
- **自动测试**:`tests/runner.html` 一键回归(启动健康/地形边界/载具视角与冷却/区块流式资源释放),详见 `tests/README.md`
...
- **植被**:树/草/岩石为跨区块共享几何+实例化批量绘制;树种按每棵树局部地貌采样(阔叶/云杉/枯树),草叶带程序化 alpha 贴图与顶点风摆动
- **地形**:Simplex 噪声分层生成高度场,按地貌混合顶点色,无限区块流式加载
- **纹理**:全部程序化生成(Canvas 绘制无缝平铺细节贴图、法线贴图、迷彩、木纹、金属磨砂、辉光、云)
- **音效**:WebAudio 实时合成(枪声、爆炸、UI、连杀音阶),零音频文件
- **模型**:代码内联几何体拼装(士兵骨骼动画、坦克、直升机、空投箱)
- **零依赖**:除 three.min.js 外无任何外部资源,离线可玩

### 为什么不换渲染库 / 可选升级路线

本项目"树太假、草太假"的根因是几何与贴图策略,而不是 Three.js 能力上限。r128 已具备
InstancedMesh、`onBeforeCompile` 着色器注入、PMREM 环境光、SSAO/Bloom/FXAA,本轮升级全部在现有
管线上完成,保持了离线、零构建、<1MB 的交付形态。若后续需要更强真实感,按需引入(均不破坏离线):

| 需求 | 建议 |
|---|---|
| 真实扫描级模型/贴图 | `GLTFLoader` + Draco/Meshopt 压缩 + KTX2 纹理(资产本地化) |
| 大场景射线检测/碰撞加速 | `three-mesh-bvh` |
| 载具真实刚体物理 | `rapier`(WASM,本地文件) |
| 引擎换代(大型化后再评估) | Babylon.js(内置物理/ GUI)或升级现代版 Three.js(需整体迁移后期管线) |
