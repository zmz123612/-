...
一款纯前端实现的开放世界第一人称射击游戏。无需安装、无需网络、无任何构建步骤——打开网页就能玩。
...
项目是纯静态页面,只需一个本地 HTTP 服务器(直接双击 `index.html` 会因浏览器 file:// 安全策略无法加载脚本):
...
浏览器打开 <http://127.0.0.1:8765> 即可。也可以把整个文件夹丢到任意静态托管上直接运行。
